...
export function Navbar() {
    ...
    return (
        <nav className="fixed flex justify-center items-center top-0 w-full z-50 transition-all duration-300 shadow-[0_4px_40px_rgba(0,0,0,0.3)] bg-white">
            ...
            {/* Mobile menu */}
            <div
                className={`md:hidden fixed inset-0 z-30 bg-white transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out h-screen`}
            >
                <div className="flex flex-col items-center justify-center h-full space-y-8 font-space relative z-10 text-black">
                    ...
                    <MobileNavLink href="#services" onClick={closeMenu}>Service</MobileNavLink>
                    <MobileNavLink href="#case-studies" onClick={closeMenu}>CaseStudies</MobileNavLink>
                    <MobileNavLink href="#projects" onClick={closeMenu}>Blog</MobileNavLink>
                    <MobileNavLink href="#development-approach" onClick={closeMenu}>How It Works</MobileNavLink>
                    <MobileNavLink href="#hire" onClick={closeMenu}>Hire</MobileNavLink>
                </div>
            </div>
        </nav>
    )
